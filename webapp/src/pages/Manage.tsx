...
type RecoverState =
  | { status: "idle" }
  | { status: "loading" }
  | { status: "success"; slug: string; managementToken: string }
...
function RecoverTokenDialog({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/api/submissions/${cleanSlug}/recover-token`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        if (res.status === 403) {
          setState({
            status: "error",
            message: "Email doesn't match the registered owner of this slug.",
          });
          return;
        }
        if (res.status === 404) {
          setState({
            status: "error",
            message: "No API found with that slug.",
          });
          return;
        }
        const json = await res.json().catch(() => null);
        const message =
          json?.error?.message ?? `Recovery failed (status ${res.status}).`;
  ...
  function handleCopyToken() {
    ...
    navigator.clipboard.writeText(state.managementToken).then(() => {
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    });
  }
  ...
  function handleDownload() {
    ...
    const content = [
      ...
      "",
      `Slug:              ${state.slug}`,
      `Management Token:  ${state.managementToken}`,
      "",
    ...
    a.href = url;
    a.download = `mpp32-${state.slug}-credentials.txt`;
    a.click();
  ...
  function handleUseToken() {
    if (state.status !== "success") return;
    onRecovered(state.slug, state.managementToken);
    onOpenChange(false);
  }

  const success = state.status === "success";
  const maskedToken = success
    ? state.managementToken.slice(0, 8) +
      "•".repeat(24) +
      state.managementToken.slice(-8)
    : "";
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <code className="font-mono text-sm text-mpp-amber break-all block bg-[#0d0d0d] border border-mpp-border rounded px-3 py-2 leading-relaxed">
                {tokenVisible ? state.managementToken : maskedToken}
              </code>
            </div>
...
function EditSettings({
  ...
  // FIX 10: client-side EVM address validation
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          ...
          {/* FIX 11: helper text for pathUSD */}
          <p className="mt-1.5 text-xs text-muted-foreground">
            Charged per request in pathUSD (USD-pegged stablecoin on Tempo). Typical: 0.001–0.1.
          </p>
        </div>
        <div>
          <label className="block mb-1.5 font-mono text-xs text-foreground uppercase tracking-wide">
            ...
            {statsQuery.isLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <StatCard
                  icon={DollarSign}
                  label="Est. Revenue"
                  value={`${stats.estimatedRevenue.toFixed(4)} pathUSD`}
                  sub={
                    stats.pricePerQuery !== null
                      ? `${stats.pricePerQuery} pathUSD × ${stats.queryCount} queries`
                      : undefined
                  }
                />
                <StatCard
