...
function VerificationCard({ auth, submission }: { auth: AuthState; submission: SubmissionResponse }) {
  ...
  const [showSetup, setShowSetup] = useState(!submission.isVerified);
  ...
  const endpointOrigin = submission.endpointUrl
    ...
    : "";
  const verifyUrl = endpointOrigin ? `${endpointOrigin}/api/mpp32-verify` : "";
  const isSuspended = (details?.verifyFailCount ?? 0) >= 3 && !submission.isVerified;
  const token = details?.verificationToken ?? "<your-token>";

  function getTroubleshootHint(msg: string): string | null {
    if (msg.includes("mismatch")) return "The response body did not match your token. Make sure you return the token as plain text with no extra characters, JSON, or HTML.";
    if (msg.match(/HTTP \d/) || msg.includes("status")) return "Your server returned an unexpected status code. Verify the route is registered and returns HTTP 200.";
    if (msg.includes("not respond") || msg.includes("Could not reach") || msg.includes("timed out")) return "Your endpoint could not be reached. Check that it is publicly accessible over HTTPS and responds within 8 seconds.";
    return null;
  }
  ...
  return (
    <div className={cn(
      ...
      <div className="px-5 py-4 space-y-4">
        ...
        {isSuspended && (
          <div className="flex items-start gap-3 px-4 py-3 bg-red-500/5 border border-red-500/20 rounded">
            ...
            <div className="text-xs text-red-300 leading-relaxed space-y-1">
              <p className="font-medium">Proxy traffic suspended after 3 consecutive verification failures.</p>
              <p>Fix the issue below, then click Verify Now to restore traffic.</p>
            </div>
          </div>
        )}
        ...
        {/* Step-by-step guide for unverified */}
        {!submission.isVerified && (
          <div className="flex flex-col gap-2 px-4 py-3 bg-blue-500/5 border border-blue-500/20 rounded">
            <p className="text-xs font-medium text-blue-400 uppercase tracking-wide">How to verify</p>
            <div className="space-y-2">
              {[
                "Copy the verification token below.",
                "Add a route to your server that returns it as plain text at the well-known URL (see examples).",
                "Click Verify Now. Once verified, proxy traffic flows to your endpoint.",
              ].map((step, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span className="flex items-center justify-center w-5 h-5 rounded-full bg-blue-500/20 text-blue-400 text-[10px] font-bold shrink-0 mt-px">{i + 1}</span>
        ...
        {/* Collapsible setup details (open by default when unverified) */}
        <Collapsible open={showSetup} onOpenChange={setShowSetup}>
          {submission.isVerified && (
            <CollapsibleTrigger className="flex items-center gap-2 text-xs text-muted-foreground hover:text-foreground transition-colors">
              ...
              {showSetup ? "Hide setup details" : "Show setup details"}
            </CollapsibleTrigger>
          )}
          <CollapsibleContent className="space-y-4">
            ...
            {/* Response format callout */}
            <div className="flex items-start gap-2.5 px-3 py-2.5 bg-amber-500/5 border-l-2 border-amber-500/50 rounded-r">
              <Info className="w-3.5 h-3.5 text-amber-400 shrink-0 mt-0.5" />
              <p className="text-xs text-muted-foreground leading-relaxed">
                Your server must return <span className="text-foreground font-medium">HTTP 200</span> with the token as{" "}
                <span className="text-foreground font-medium">plain text</span> — no JSON, no HTML, no wrapper. Just the raw token string.
              </p>
            </div>
            ...
            <div>
              ...
              <Tabs defaultValue="express" className="w-full">
                <TabsList className="w-full bg-[#0d0d0d] border border-mpp-border h-9">
                  <TabsTrigger value="express" className="text-xs data-[state=active]:bg-mpp-card">Express.js</TabsTrigger>
                  <TabsTrigger value="flask" className="text-xs data-[state=active]:bg-mpp-card">Python</TabsTrigger>
                  <TabsTrigger value="static" className="text-xs data-[state=active]:bg-mpp-card">Static File</TabsTrigger>
                  <TabsTrigger value="test" className="text-xs data-[state=active]:bg-mpp-card flex items-center gap-1">
                    <Terminal className="w-3 h-3" /> Test
                  </TabsTrigger>
                </TabsList>
                <TabsContent value="express">
                  <div className="bg-[#0d0d0d] border border-mpp-border rounded px-3 py-2 relative group">
  ...
  res.type('text/plain').send('${token}');
                    ...
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <CopyButton text={`app.get('/api/mpp32-verify', (req, res) => {\n  res.type('text/plain').send('${token}');\n});`} />
                    </div>
                  </div>
                </TabsContent>
...
def verify():
    return '${token}', 200, {'Content-Type': 'text/plain'}`}</pre>
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <CopyButton text={`@app.route('/api/mpp32-verify')\ndef verify():\n    return '${token}', 200, {'Content-Type': 'text/plain'}`} />
                    </div>
                  </div>
                </TabsContent>
...
location = /api/mpp32-verify {
  ...
  return 200 '${token}';
                    ...
                    <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                      <CopyButton text={`location = /api/mpp32-verify {\n  default_type text/plain;\n  return 200 '${token}';\n}`} />
                    </div>
                  </div>
                </TabsContent>
                <TabsContent value="test">
                  <div className="space-y-2">
                    <p className="text-xs text-muted-foreground">Run this command to test before clicking Verify:</p>
                    <div className="bg-[#0d0d0d] border border-mpp-border rounded px-3 py-2 relative group">
                      <pre className="font-mono text-xs text-foreground leading-relaxed whitespace-pre-wrap">{`curl -s ${verifyUrl || "https://yourdomain.com/api/mpp32-verify"}`}</pre>
                      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <CopyButton text={`curl -s ${verifyUrl || "https://yourdomain.com/api/mpp32-verify"}`} />
                      </div>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      Should output <span className="text-foreground font-mono">exactly</span> your verification token — nothing else.
                    </p>
                  </div>
                </TabsContent>
              </Tabs>
            ...
            >
              ...
              Full verification guide in docs
            </Link>
          </CollapsibleContent>
        </Collapsible>
        ...
        {verifyState === "success" && verifyMessage !== null && (
          <div className="flex items-start gap-2 px-3 py-2.5 bg-green-500/5 border border-green-500/20 rounded">
        ...
        {verifyState === "error" && verifyMessage !== null && (
          <div className="space-y-2">
            <div className="flex items-start gap-2 px-3 py-2.5 bg-red-500/5 border border-red-500/20 rounded">
              <XCircle className="w-3.5 h-3.5 text-red-400 shrink-0 mt-0.5" />
              <div className="space-y-1.5">
                <p className="text-xs text-red-400">{verifyMessage}</p>
                {getTroubleshootHint(verifyMessage) && (
                  <p className="text-xs text-muted-foreground">{getTroubleshootHint(verifyMessage)}</p>
                )}
              </div>
            </div>
            {verifyUrl && (
              <div className="px-3">
                <p className="text-xs text-muted-foreground mb-1">Debug with:</p>
                <div className="flex items-center gap-2">
                  <code className="font-mono text-xs text-foreground bg-[#0d0d0d] border border-mpp-border rounded px-2 py-1 flex-1">
                    curl -s {verifyUrl}
                  </code>
                  <CopyButton text={`curl -s ${verifyUrl}`} />
                </div>
              </div>
            )}
          </div>
