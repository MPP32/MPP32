            ...
            <Section id="security" title="9. Security Model">
              ...
              <div className="space-y-3">
                {[
                  ...
                  ["Facilitator based settlement (x402)", "x402 payments are verified through PayAI's facilitator at facilitator.payai.network, which natively supports Solana mainnet (solana:5eykt4UsFv8P8NJdTREpY1vzqKqZKvdp). The verify endpoint validates the payment proof. The settle endpoint finalizes the transaction. Both calls use timeouts (10 seconds for verify, 15 seconds for settle) to prevent hanging connections. A startup probe refuses to boot the backend in production if the configured facilitator does not advertise support for the configured network, and a per-request failover routes to a configured backup facilitator on transport errors."],
