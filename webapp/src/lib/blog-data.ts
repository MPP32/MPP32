...
The client's MPP SDK (such as mppx for JavaScript or pympp for Python) parses the challenge, constructs a payment transaction for the specified amount in pathUSD, and broadcasts it to the payment network. On confirmation, the SDK receives a signed receipt JWT derived from the on-chain transaction.
...
## How Tempo and pathUSD Fit In

Tempo is the payment layer MPP uses by default. It operates as an Ethereum L2 settlement network with its own stablecoin, pathUSD, pegged 1:1 to USD. Settlement is fast (under one second for most transactions) and fees are negligible at the micropayment scale relevant to API calls.

pathUSD serves as the unit of account across MPP-compatible APIs. When you see a price like 0.008 pathUSD per query, that is $0.008 — eight-thousandths of a dollar. The stability of the peg is what makes predictable per-call pricing possible.
...
At 0.008 pathUSD per query, the friction of payment is negligible relative to the intelligence value returned. An analyst running 100 queries in a session spends $0.80. An automated pipeline running 10,000 queries a day spends $80. The math works at every scale without requiring tiered pricing or negotiated contracts.
